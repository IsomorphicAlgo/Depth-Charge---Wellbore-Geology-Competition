...
- **`wellbore_submission.py`** — **Milestone 4** test inference: default train/test roots from **`wellbore_cv`**, LightGBM fit (or ``--model-in`` pickle), **1-based MD-sorted foot slicing** aligned to **`data/sample_submission.csv`**, length/id-order assertions, optional **tvt** rounding. CLI: ``python wellbore_submission.py --help`` (writes e.g. ``results/submission.csv``).
- **`wellbore_submission.py`** — **Milestone 4** test inference: build the V3 tabular stack on **`data/test`** (or **`data/tidytest`** when it contains laterals), optionally fit a full-data LightGBM on **`train_tidy`**, and write **`id,tvt`** in lockstep with **`data/sample_submission.csv`** (length, id order, optional rounding). CLI: `python wellbore_submission.py --help`.
